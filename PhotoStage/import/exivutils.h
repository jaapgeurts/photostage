...
template <typename T> Nullable<T> getExifValue(Exiv2::ExifData&, const QString&)
{
...
template <>
Nullable<QString> getExifValue(Exiv2::ExifData& data, const QString& key)
{
  try
  {
                                                      ...
                                                      const QString&   key)
{
  ...
  try
  {
    ...
    if (pos != data.end() && pos->size() > 0)
      return Nullable<PhotoStage::ExifInfo::Rotation>(
          (PhotoStage::ExifInfo::Rotation)pos->toLong());
  }
  catch (Exiv2::BasicError<char> e)
  {
    // qDebug() << "No such Exif field" << key << "Debug:" << e.what();
...
bool getExifBytes(Exiv2::ExifData& data, const QString& key, uint8_t* buf,
                  size_t size)
{
  Exiv2::ExifData::const_iterator pos;
