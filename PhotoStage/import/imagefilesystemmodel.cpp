...
ImageFileSystemModel::ImageFileSystemModel(QObject* parent)
    : QAbstractListModel(parent), mRootPath(), mFilters(), mIncludeSubdirs(true)
{
  ...
  switch (role)
  {
    ...
    case Widgets::AbstractTile::ImageRole:
      ...
      else
      {
        ...
        connect(loader, &PreviewFileLoader::dataReady, this,
                &ImageFileSystemModel::imageLoaded);
        mThreadPool->start(loader);

        // Insert the dummy image here so that the we know the loader thread has
        // been started
                                       ...
                                       const QImage&      pixmap)
{
  ...
  info.isInLibrary = DatabaseAccess::photoDao()->IsInLibrary(
      computeImageFileHash(info.filePath));
  info.image = pixmap;
  mPreviewInfoCache->insert(index, info);
  QVector<int> roles;
...
{
  QDirIterator iter(mRootPath, mFilters,
                    QDir::Dirs | QDir::Files | QDir::NoDotAndDotDot |
                        QDir::NoSymLinks,
                    mIncludeSubdirs ? QDirIterator::Subdirectories
