...
template <typename T> static inline T max3(T a, T b, T c)
{
  if (a > b)
    return a > c ? a : c;
  else
...
bool Exiv2Lib::openData(const QByteArray& data)
{
  ...
  {
    mImageFile =
        ImageFactory::open((const byte*)data.constData(), (long)data.length());
...
bool Exiv2Lib::populateExifInfo()
{
  ...
  mExifInfo.isoSpeed =
      getExifValue<uint8_t>(data, "Exif.Photo.ISOSpeedRatings");
  mExifInfo.dateTimeOriginal =
      getExifValue<QDateTime>(data, "Exif.Photo.DateTimeOriginal");
  mExifInfo.dateTimeDigitized =
      getExifValue<QDateTime>(data, "Exif.Photo.DateTimeDigitized");
  ...
  Nullable<ExifInfo::Rotation> rot =
      getExifValue<ExifInfo::Rotation>(data, "Exif.Image.Orientation");
...
void Exiv2Lib::readNikonNotes(ExifData& data)
{
  ...
  if (getExifBytes(data, "Exif.Nikon3.Lens", (uint8_t*)lens,
                   4 * sizeof(double)))
  {
    mExifInfo.lensName = QString("%1-%2 ƒ/%3-%4")
                             .arg(lens[0], 0, 'f', 0)
                             .arg(lens[1], 0, 'f', 0)
                             .arg(lens[2], 0, 'f', 0)
                             .arg(lens[3], 0, 'f', 0);
  ...
  if (pos != data.end())
  {
    if (*mExifInfo.model == "NIKON D300")
    {
      ...
      if (pos != data.end())
      {
        ...
        wb[1] = 1.0f;
        wb[2] = pos->toFloat(1);
        qDebug() << "Nikon: RGB" << wb[0] << "," << wb[1] << "," << wb[2];
      }
    }
...
void Exiv2Lib::setWhiteBalanceCoeffsCanon(ExifData& data, float wb[3])
{
  ...
  if (pos != data.end())
  {
    ...
    if (*mExifInfo.model == "Canon EOS 350D DIGITAL") // || EOS 20D
    {
      ...
      //                                    colorData->V1.WhiteBalanceTable[WB_AsShot].RGGB[2];
      //                                    wb[1] =
      //                                    ((colorData->V1.WhiteBalanceTable[WB_AsShot].RGGB[1]+colorData->V1.WhiteBalanceTable[WB_AsShot].RGGB[3])/2);
      //                                    wb[2] =
      //                                    colorData->V1.WhiteBalanceTable[WB_AsShot].RGGB[0];
      wb[0] = colorData->ColorData.V1.WhiteBalanceTable[WB_AsShot].RGGB[0];
      wb[1] = ((colorData->ColorData.V1.WhiteBalanceTable[WB_AsShot].RGGB[1] +
                colorData->ColorData.V1.WhiteBalanceTable[WB_AsShot].RGGB[2]) /
               2);
    ...
    else if (*mExifInfo.model == "Canon PowerShot S30")
    {
      ...
      wb[1] = (colorData->ColorData.V5.WhiteBalanceTable[WB_AsShot].GRBG[0] +
               colorData->ColorData.V5.WhiteBalanceTable[WB_AsShot].GRBG[3]) /
              2;
    ...
    else if (*mExifInfo.model == "Canon PowerShot S110")
    {
      ...
      wb[1] = (colorData->ColorData.V3.WhiteBalanceTable[WB_AsShot].RGGB[1] +
               colorData->ColorData.V3.WhiteBalanceTable[WB_AsShot].RGGB[2]) /
              2;
    ...
    else // attempt to read at the default position at V4
    {
      ...
      wb[1] = (colorData->ColorData.V7.WhiteBalanceTable[WB_AsShot].RGGB[1] +
               colorData->ColorData.V7.WhiteBalanceTable[WB_AsShot].RGGB[2]) /
              2;
