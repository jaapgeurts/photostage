...
DevelopView::DevelopView(QWidget* parent)
    ...
    mZoomMode(ZoomFit),
    mInfoMode(InfoOff),
    mWorkingMode(ModeZoom),
    cornerSize(HANDLE_WIDTH, HANDLE_WIDTH),
    vertSideSize(HANDLE_WIDTH, 2 * HANDLE_WIDTH),
    horzSideSize(2 * HANDLE_WIDTH, HANDLE_WIDTH),
    mActiveHandle(-1),
    mRotationZone(-1),
    mGridType(RuleOfThirds),
    mIsLockAspectRatio(false),
    mAspectRatio(3.0 / 2.0f),
    mCropDrag(false),
    mRotateDrag(false),
    mRotationAngle(0.0f),
...
void DevelopView::setTransform()
{
    ...
    mProjection.translate((width() - mPhoto.width()) / 2, (height() - mPhoto.height()) / 2);
}
...
void DevelopView::createCropFrame(const QRect& rect)
{
    ...
    // left
    r.moveTo(cropFrame.left(), cropFrame.top() + (cropFrame.height() - vertSideSize.height()) / 2);
    mCropFrame.append(r);

    // right
    r.moveTo(cropFrame.right() - HANDLE_WIDTH + 1, cropFrame.top() + (cropFrame.height() - vertSideSize.height()) / 2);
    mCropFrame.append(r);

    r.setSize(horzSideSize);
    // top
    r.moveTo(cropFrame.left() + (cropFrame.width() - horzSideSize.width()) / 2, cropFrame.top());
    mCropFrame.append(r);

    //bottom
    r.moveTo(cropFrame.left() + (cropFrame.width() - horzSideSize.width()) / 2, cropFrame.bottom() - HANDLE_WIDTH + 1);
    mCropFrame.append(r);
}
...
void DevelopView::processCropHandleDrag(QMouseEvent* event)
{
    ...
    {
        float newHeight;
        bool  isLandscape = (mActiveHandle == SIDE_LEFT
            || mActiveHandle == SIDE_BOTTOM
            || mActiveHandle == SIDE_RIGHT
            || mActiveHandle == SIDE_TOP)
            ? mOrigCropRect.width() > mOrigCropRect.height() : rect.width() > rect.height();
