...
void TileView::setModel(QAbstractItemModel* model)
{
  ...
  connect(model, &QAbstractItemModel::rowsInserted, this,
          &TileView::onRowsInserted);
  connect(model, &QAbstractItemModel::rowsRemoved, this,
          &TileView::onRowsRemoved);
...
void TileView::paintEvent(QPaintEvent* /*event*/)
{
  ...
  int innerLength; // width or height - scrollbar. depending on horiz or vert
                   // orientation
  int start;
  int end;
  int offset;
  ...
  if (mOrientation == Qt::Vertical)
  {
    ...
    offset =
        mViewportPosition % mComputedCellHeight; // portion of the row that is
  ...
  else // Horizontal
  {
    ...
    offset =
        mViewportPosition % mComputedCellWidth; // portion of the row that is
                                                // outside the view (on top)
    end =
        start + mTilesPerColRow * ((width() + offset) / mComputedCellWidth + 1);
  }
  ...
  for (int i = start; i <= end; i++)
  {
    ...
    if (mIsCheckBoxMode)
    {
      if (mCheckedList->contains(itemIndex))
        ...
        tileInfo.tileState = (TileInfo::TileState)(
            tileInfo.tileState | TileInfo::TileStateUnchecked);
    }
    ...
    // qDebug() << "cell:" << i <<
    // "("<<xpos<<","<<ypos<<","<<mComputedCellWidth<<","<<mComputedCellHeight<<")";
    ...
    if (mIsCheckBoxMode)
    {
      ...
      option.state |= mCheckedList->contains(itemIndex) ? QStyle::State_On
                                                        : QStyle::State_Off;
...
/*
   void TileView::mouseMoveEvent(QMouseEvent* event)
   {
    ...
    // check for transition from one tile to another and send mouseEnter and
   mouseLeave events
   }*/
...
int TileView::posToIndex(const QPoint& pos) const
{
  ...
  if (mOrientation == Qt::Vertical)
  {
    ...
    pageStart =
        (mViewportPosition - yoff) / mComputedCellHeight * mTilesPerColRow;
  ...
  else
  {
    ...
    // skip the images that are scrolled out of the view (on the top)
    pageStart =
        (mViewportPosition - xoff) / mComputedCellWidth * mTilesPerColRow;
