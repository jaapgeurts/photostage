...
PathItem* PathDAO::getPathItems()
{
  ...
  {
    rootItem =
        new PathItem(query.value(0).toLongLong(), query.value(1).toString(),
...
void PathDAO::getPathItemsRec(PathItem* root)
{
  QSqlQuery query;
  QString   queryText =
      QString("select p.id, p.directory, p.parent_id, count(f.id) \
...
// This function will check the path hierarchy in the database and insert the
// necessary paths it returns the path id of the last directory
int PathDAO::createPaths(QStringList& paths)
{
  QSqlQuery q;

  q.prepare("select id, directory ,parent_id from path where directory = :dir "
            "and ifnull(parent_id,-1) = :p_id");
...
int PathDAO::insertPathRec(QSqlQuery& q, const QStringList& path, int pos,
                           int parentid)
{
  if (pos >= path.length())
    return parentid;
  ...
  if (q.exec())
  {
    ...
    else
    {
      ...
      iq.prepare("insert into path (directory,parent_id) values (:directory, "
                 ":parent)");
