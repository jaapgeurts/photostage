...
{
KeywordDAO::KeywordDAO(QObject* parent)
    : QObject(parent)
{
}
...
void KeywordDAO::getKeywordItemsRec(KeywordItem* root)
{
    ...
    QString   queryText = QString(
        "select id,keyword,parent_id from keyword where parent_id = :parent_id order by keyword;");
...
void KeywordDAO::assignKeywords(const QStringList& words, const QList<Photo>& list)
{
    if (list.isEmpty())
        return;
...
void KeywordDAO::unAssignKeywordsExcept(const QStringList& words, const QList<Photo>& list)
{
    if (list.isEmpty())
        return;
...
QMap<QString, int> KeywordDAO::getPhotoKeywordsCount(const QList<Photo>& list) const
{
    QSqlQuery q;
    QString   query =
