...
// All three collections (WORK = temporary collection, USER = user defined
// collection, IMPORT = import collections Each tree root node uses these
// respective names
...
CollectionDAO::CollectionDAO(QObject* parent) : QObject(parent)
{
}
...
CollectionItem* CollectionDAO::getCollectionItems(CollectionSources sources)
{
  ...
  QSqlQuery q;
  QString query = "select id, name, parent_id from collection where parent_id "
                  "is NULL and ( 1=0 ";
...
void CollectionDAO::getCollectionItemsRec(CollectionItem* root, long long id,
                                          CollectionSource source)
{
  QSqlQuery query;
  QString   queryText =
      QString("select c.id, c.name,c.parent_id, count(cf.photo_id) \
...
long long
CollectionDAO::addCollectionInternal(const Nullable<long long>& parentid,
                                     ...
                                     const QString&             name)
{
  ...
  if (parentid == nullptr)
  {
    // add as a root item. make sure there is a sentinel root item first named
    // USER. These are the user created collections get the root id ( a single
    // root is needed )
    q.clear();
    q.prepare(
        "select id from collection where parent_id is NULL and name = :name;");
  ...
  q.finish();
  q.prepare(
      "insert into collection (name,parent_id) values ( :name , :parent )");
...
void CollectionDAO::addPhotosToCollection(long long               collectionId,
                                          const QList<long long>& photoIds)
{
  // TODO: wrap this in transaction
  DatabaseAccess::instance()->beginTransaction();
  QSqlQuery q;
  QString query("insert into collection_photo (collection_id,photo_id) values "
                "(:id,:photo_id);");
...
long long CollectionDAO::addCollection(const Nullable<long long>& parentid,
                                       const QString&             name)
{
...
void CollectionDAO::removePhotosFromCollection(long long           collectionid,
                                               const QList<Photo>& list)
{
  QSqlQuery q;

  QString query = "delete from collection_photo where collection_id = :cid and "
...
long long CollectionDAO::rebuildCollectionTree(long long parent_id,
                                               long long left)
{
