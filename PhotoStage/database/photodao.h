...
public:
  ...
  void assignGeoCoordinate(const QGeoCoordinate& coord,
                           const QList<Photo>&   list);

  void importPhoto(long long collectionid, int pathId, const QString& fileName,
                   long long hash, const ExifInfo& exifInfo);

  // Photo Items are created here.
  // ownership is transferred to the caller who should call delete on the
  // objects
  QList<Photo> getPhotosByPath(long long path_id,
                               bool      includeSubDirs = true) const;

  QList<Photo> getPhotosByCollectionId(long long collection_id,
...
private:
  ...
  void getLeftRightForPathId(long long path_id, long long& lft,
                             long long& rgt) const;
  void getLeftRightForCollectionId(long long collection_id, long long& lft,
                                   long long& rgt) const;
};
} // namespace PhotoStage
