...
template <typename T> Nullable<T>::Nullable() : m_hasValue(false), m_value(T())
{
}

template <typename T>
Nullable<T>::Nullable(const T& value) : m_hasValue(true), m_value(value)
{
}
...
template <typename T> bool operator==(const Nullable<T>& op, const T& value)
{
  if (!op.m_hasValue)
    return false;
...
template <typename T> bool operator==(const T& value, const Nullable<T>& op)
{
  if (!op.m_hasValue)
    return false;
...
template <typename T> const Nullable<T>& Nullable<T>::operator=(const T& value)
{
  m_hasValue = true;
  m_value    = value;
  return *this;
