...
template <typename T> static inline long max(T x, T y)
{
  return x > y ? x : y;
}

template <typename T> static inline long min(T x, T y)
{
  return x < y ? x : y;
}

template <typename T> static inline long clip(T v, T x, T y)
{
...
CameraMetaData* Metadata::metaData()
{
  if (mMetaData == NULL)
  {
    ...
    {
      QString filePath =
          QCoreApplication::applicationDirPath() + "/../Resources/cameras.xml";
...
RawIO::RawIO(const QByteArray&                           memFile,
...
bool RawIO::readMatrix(const QString& model, float* mat) const
{
  QFile f(QCoreApplication::applicationDirPath() +
          "/../Resources/camera_color_matrix.json");
  ...
  for (int i = 0; i < cameraList.size(); ++i)
  {
    ...
    for (int j = 0; j < modelAliases.size(); j++)
    {
      ...
      if (modelName.compare(model, Qt::CaseInsensitive))
      {
        ...
        {
          qDebug()
              << "Currently only 3x3 color matrices are supported (= 9 values)";
          return false;
        }
                          ...
                          const QString& cameraModel)
{
  ...
  std::unique_ptr<const Buffer> map(
      new Buffer((const uchar8*)memFile.constData(), memFile.length()));
      ...
      bytes_per_pixel == 2)
  {
    ...
    if (!cameraFound)
    {
      qDebug() << "Camera model" << cameraModel
               << "not found. Using identity matrix.";
