...
PhotoSortFilterProxyModel::PhotoSortFilterProxyModel(QObject* parent)
    : QSortFilterProxyModel(parent), mFilterInfo()
{
}

bool PhotoSortFilterProxyModel::filterAcceptsRow(
    int source_row, const QModelIndex& /*source_parent*/) const
{
  Photo p = sourceModel()
                ->data(sourceModel()->index(source_row, 0), Photo::DataRole)
                .value<Photo>();
  ...
  if (!mFilterInfo.colorBlue && !mFilterInfo.colorGreen &&
      !mFilterInfo.colorRed && !mFilterInfo.colorYellow)
  {
...
QList<Photo>
PhotoSortFilterProxyModel::toList(const QItemSelection& selection) const
{
  QList<Photo> list;
