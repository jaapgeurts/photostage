...
Cartography::Cartography(QAbstractItemModel* model, QWidget* parent)
    : Module(parent), ui(new Ui::Map), mPhotoModel(model), mLoadPhoto(false),
      mSelectionModel(NULL), mSavedCoordinate(), mZoomLevel(2)
{
  ui->setupUi(this);
  ...
  connect(ui->mapView, &MapView::MapView::zoomLevelChanged, this,
          &Cartography::onZoomLevelChanged);
  connect(ui->mapView, &MapView::MapView::centerChanged, this,
          &Cartography::onCenterChanged);
  connect(ui->mapView, &MapView::MapView::contextMenuRequested, this,
          &Cartography::onContextMenuRequested);
  ...
  if (settings.contains(SETTINGS_LATITUDE) &&
      settings.contains(SETTINGS_LONGITUDE))
  {
...
void Cartography::setSelectionModel(QItemSelectionModel* selectionModel)
{
  ...
  connect(selectionModel, &QItemSelectionModel::currentChanged, this,
          &Cartography::onCurrentPhotoChanged);
  connect(selectionModel, &QItemSelectionModel::selectionChanged, this,
          &Cartography::onSelectionChanged);
  mLayer->setSelectionModel(selectionModel);
}

void Cartography::onCurrentPhotoChanged(const QModelIndex& current,
...
void Cartography::onSelectionChanged(const QItemSelection& /*selected*/,
...
void Cartography::onContextMenuRequested(const QGeoCoordinate& coord,
                                         const QPoint&         pos)
{
