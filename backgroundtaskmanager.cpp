...
void BackgroundTaskManager::addRunnable(BackgroundTask* const task)
{
    ...
    connect(task, &BackgroundTask::taskFinished,
        this, &BackgroundTaskManager::taskFinished);
    connect(task, &BackgroundTask::progressUpdated,
        progress, &BackgroundTaskProgress::updateProgress);

    // connection interface signal to the task
    connect(progress, &BackgroundTaskProgress::cancelClicked,
        task, &BackgroundTask::cancel);
